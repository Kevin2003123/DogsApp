...
const DogsForm = () => {
  ...
  const handleAddTag = (event) => {
    ...
    if (!temperamentTags.some((x) => x.id === newTag.id))
      if (newTag.id !== "0" && newTag.id)
        setTemperamentTags([...temperamentTags, newTag]);
  };
  ...
  const validationHandler = () => {
    let newValidation = {};
    if (/\d+/.test(name) || !name)
      newValidation.name = "Name can't be empty and can't contain numbers";
    ...
    if (temperamentTags.length === 0)
      newValidation.temp = "the dog need have at least one temperament";
  ...
  const submitHandler = async (e) => {
    ...
    if (
      ...
      await axios.post("http://localhost:3001/dogs", {
        ...
        minLife,
        maxLife,
        temperaments: [...temperamentTags].map((x) => Number(x.id)),
      });
